...
export function TransferForm({ reservationId, currentName }: { reservationId: string; currentName: string }) {
  ...
  function submit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const r = await transferReservationAction({ reservationId, owner, name, email, phone });
    ...
    <details className="faq group">
      ...
      <form onSubmit={submit} className="mt-4 grid gap-3">
        ...
        <input
          className="input"
          type="email"
          aria-label="Tu email, el de la reserva"
          placeholder="Tu email (con el que reservaste)"
          value={owner}
