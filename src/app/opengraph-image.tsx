...
export default async function Image() {
  ...
  const when = event ? `de ${formatMonth(event.date)} · ${formatTime(event.date)} hs` : "Una mesa larga en una casa de La Plata";
  ...
  const text = `${SITE_NAME}${headline}${when}${sub}${menu}Cena a puertas cerradas · La PlataCinco pasos, cada plato con su tragoAgotado · abrimos la próxima fecha0123456789`;
    ...
    (
      ...
      >
        ...
        <div style={{ display: "flex", fontSize: 24, marginTop: 40, color: "#c9a96e", letterSpacing: 4 }}>
          Cinco pasos, cada plato con su trago
        </div>
      </div>
    ),
    {
