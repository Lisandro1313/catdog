...
/**
 * La pantalla de la cocina: solo los pasos que pidieron las mesas, en orden de llegada y en letra
 * grande. Un toque los marca servidos. Pensada para el celular o la tablet de Agustín, apoyada.
 */
export default async function CocinaPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const event = await prisma.event.findUnique({ where: { id }, select: { id: true, title: true } });
  if (!event) notFound();
  const pendientes = await getPendientes(id, "cocina");
  const ahora = nowMs();
    ...
    <div className="ap fixed inset-0 z-50 overflow-y-auto bg-bg px-5 pt-6 text-ink" style={{ paddingBottom: "calc(2rem + env(safe-area-inset-bottom))" }}>
      ...
      <div className="mx-auto w-full max-w-3xl">
        <div className="flex items-baseline justify-between">
          <div>
            <p className="ap-eyebrow">✦ {SITE_NAME} · cocina ✦</p>
        ...
        {pendientes.length === 0 ? (
          <p className="mt-16 text-center text-xl text-muted">Nada pendiente. Cuando una mesa pida su paso, aparece acá.</p>
        ) : (
